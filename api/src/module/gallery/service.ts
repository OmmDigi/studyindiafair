...
const COLUMNS = "id, category_id, media_type, image_path, youtube_id, alt_text, position, is_active, created_at, updated_at";
const SELECT = `SELECT ${COLUMNS} FROM gallery_items`;
...
export async function listPublic({ category, page, limit }: PublicListQuery) {
  ...
  const [{ rows }, count] = await Promise.all([
    query(
      `SELECT i.id, i.media_type, i.image_path, i.youtube_id, i.alt_text, i.position, c.id AS category_id, c.name AS category_name, c.slug AS category_slug
...
export async function getById(id: number) {
  ...
  if (!rows[0]) throw new AppError(404, "Gallery item not found");
  return rows[0];
}
...
export async function create({ category_id, is_active, items }: CreateGalleryInput, actorId: number) {
  const { rows } = await run(() =>
    query<Row>(
      `INSERT INTO gallery_items (category_id, media_type, image_path, youtube_id, alt_text, position, is_active, created_by, updated_by)
       SELECT $1, v.media_type, v.image_path, v.youtube_id, v.alt_text, ${NEXT_POSITION} + v.ord, $6, $7, $7
       FROM unnest($2::text[], $3::text[], $4::text[], $5::text[]) WITH ORDINALITY AS v(media_type, image_path, youtube_id, alt_text, ord)
       RETURNING ${COLUMNS}`,
      [
        category_id,
        items.map((i) => i.media_type),
        items.map((i) => (i.media_type === "image" ? i.image_path : null)),
        items.map((i) => (i.media_type === "youtube" ? i.youtube_url : null)),
        items.map((i) => i.alt_text),
        is_active,
        actorId,
      ]
    )
...
export async function update(id: number, input: UpdateGalleryInput, actorId: number) {
  ...
  const mediaType = input.media_type ?? current.media_type;
  const imagePath = mediaType === "image" ? (input.image_path ?? current.image_path) : null;
  const youtubeId = mediaType === "youtube" ? (input.youtube_url ?? current.youtube_id) : null;
  if (mediaType === "image" && !imagePath) throw new AppError(422, "Image is required");
  if (mediaType === "youtube" && !youtubeId) throw new AppError(422, "A valid YouTube URL is required");
  const moved = categoryId !== current.category_id;
  await run(() =>
    query(
      `UPDATE gallery_items SET category_id = $1, media_type = $2, image_path = $3, youtube_id = $4, alt_text = $5, is_active = $6,
       position = ${moved ? `${NEXT_POSITION} + 1` : "position"}, updated_by = $7, updated_at = NOW() WHERE id = $8`,
      [
        categoryId,
        mediaType,
        imagePath,
...
export async function remove(id: number) {
  const { rows } = await query<{ image_path: string | null }>("DELETE FROM gallery_items WHERE id = $1 RETURNING image_path", [id]);
  if (!rows[0]) throw new AppError(404, "Gallery item not found");
